	...
	$.Cassette.prototype 	= {
		...
		_play				: function() {
			...
			if( data ) {
					...
					var currentSong = _self._getSide().current.getSong( data.songIdx );
					_self._prepare( currentSong );
					_self._updateSongDisplay( currentSong, _self.currentSide );
		...
		_updateSongDisplay	: function( song, side ) {

			if( this.$songTitle && this.$songSide ) {
				
				var songName = song ? song.name.replace(/_/g, ' ').replace(/BlueDucks/g, 'Blue Ducks').replace(/BlankKyt/g, 'Blank Kytt') : 'Ready to play';
				var sideText = side === 1 ? 'Side A' : 'Side B';
